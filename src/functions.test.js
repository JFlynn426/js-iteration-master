...
const stringyIndexes = stringArray => {
  return stringArray.map(
    (string, stringIndex) => `${string} is at index ${stringIndex}`
  )
}
...
const onlyTheEvenIndexedSurvive = array => {
  return array.filter((number, numberIndex) => numberIndex % 2 === 0)
}
...
const findTheNeedle = strings => {
  return strings.filter(string => string.includes('needle')).toString()
}
...
const everyYourself = array => {
  ...
  return trueValues === array.length
}
